...
async function checkValidReferrer(req, res, next) {
  ...
  try {
    ...
    const referrerWithReferrals = await userRepository.getUserWithReferrals(referrerId);

    const referralCount = referrerWithReferrals.Referrals?.length || 0;

    if (referralCount >= 8) {
      throw new AppError('Referrer has reached the maximum of 8 referrals', 400);
    }
...
async function checkDuplicateUser (req, res, next){
  ...
  try {
    ...
    return next(new AppError('Database error while checking for duplicate user', 500));
  }
};
